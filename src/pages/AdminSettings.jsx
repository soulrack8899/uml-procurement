...
import { Shield, Lock, ChevronRight, Download, Users, SwitchCamera } from 'lucide-react'

const AdminSettings = () => {
   const navigate = useNavigate()
   const { currentCompany, companies, isMobile } = useCompany()
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '1.5rem' : '2.5rem', paddingBottom: '4rem' }}>
            ...
            <div style={{ gridColumn: isMobile ? 'auto' : 'span 5', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
               ...
               <section style={{ background: 'var(--surface-container-high)', padding: '1.75rem', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                     {[
                        { name: 'Standard Workflow', status: 'Enabled' },
                        { name: 'Director Override', status: 'Active' },
                        { name: 'Audit Persistence', status: 'Active' }
                     ].map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', background: 'var(--surface-container-lowest)', borderRadius: 'var(--radius-sm)' }}>
                           <span style={{ fontSize: '0.875rem', fontWeight: 700 }}>{item.name}</span>
                           <span style={{ fontSize: '0.625rem', fontWeight: 800, color: 'var(--primary)', textTransform: 'uppercase' }}>{item.status}</span>
                        </div>
                     ))}
                  </div>
         ...
         <section style={{ background: 'var(--surface-container-low)', padding: isMobile ? '1.5rem' : '2.5rem', borderRadius: 'var(--radius-sm)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: '2rem' }}>
               <div>
                  <p style={{ fontSize: '0.625rem', fontWeight: 900, color: 'var(--outline)', textTransform: 'uppercase', marginBottom: '1rem' }}>Total Log Entries</p>
                  <h3 style={{ fontSize: '1.5rem', fontWeight: 900 }}>{auditLogs.length}</h3>
                  <p style={{ fontSize: '0.625rem', color: 'var(--primary)', fontWeight: 700, marginTop: '0.5rem' }}>Available for Review</p>
               </div>
               <div>
                  <p style={{ fontSize: '0.625rem', fontWeight: 900, color: 'var(--outline)', textTransform: 'uppercase', marginBottom: '1rem' }}>Latest Activity</p>
                  <span style={{ fontSize: '0.875rem', fontWeight: 800 }}>
                     {auditLogs.length > 0 ? new Date(auditLogs[0].timestamp).toLocaleString() : 'No activity yet'}
                  </span>
               </div>
               <div>
                  <p style={{ fontSize: '0.625rem', fontWeight: 900, color: 'var(--outline)', textTransform: 'uppercase', marginBottom: '1rem' }}>Company</p>
                  <span style={{ fontSize: '0.875rem', fontWeight: 800, color: 'var(--primary)' }}>
                     {currentCompany?.name || 'N/A'}
                  </span>
               </div>
            </div>
         ...
         <section style={{ background: 'var(--primary)', color: 'white', padding: isMobile ? '1.5rem' : '2.5rem', borderRadius: 'var(--radius-sm)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
            <div>
               ...
               <p style={{ fontSize: '0.875rem', opacity: 0.8, maxWidth: '30rem' }}>
                  Managing <strong style={{ textDecoration: 'underline' }}>{currentCompany?.name}</strong>.
                  {companies.length > 1 ? ` You have access to ${companies.length - 1} other ${companies.length - 1 === 1 ? 'company' : 'companies'}.` : ''}
               </p>
            </div>
            <button
               onClick={() => document.querySelector('header select')?.focus()}
               style={{ padding: '0.75rem 2rem', background: 'white', color: 'var(--primary)', border: 'none', borderRadius: 'var(--radius-sm)', fontWeight: 900, cursor: 'pointer' }}
            >
               Switch Company
            </button>
         </section>
